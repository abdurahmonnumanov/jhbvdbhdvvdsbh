...
function logic() {
    ...
    addCart.addEventListener('click', () => {
        ...
        korzina.forEach((item, index) => {
            receiptOut.innerHTML += `<p class = "Out"><b class = "name">${indMas[index]}.</b> ${item.name}. <b class = "piege">                                                                                                                Кол-во: </b> ${item.amount} <b class = "price"> сумма </b>  ${item.price}
            </p>`

        })
        receiptOut.innerHTML += `\n<p class = "total">Каллорийность: ${totalKcall}kal.  Общая сумма: ${totalPrice}сум</p>`
